...
Project.prototype.install = function (decEndpoints, options, config) {
    ...
    .spread(function (json, tree) {
        ...
        // and no partm.json is present in current directory
        if (!that._jsonFile && decEndpoints.length === 0 ) {
            throw createError('No partm.json present', 'ENOENT');
        }
...
Project.prototype.saveJson = function (forceCreate) {
    ...
    //console.log(this._jsonFile)
    // Error out if the json file does not exist, unless force create
    // is true
    if (!this._jsonFile && !forceCreate) {
        this._logger.warn('no-json', 'No partm.json file to save to, use bower init to create one');
        return Q.resolve();
    }

    file = this._jsonFile || path.join(this._config.cwd, 'partm.json');
...
Project.prototype._readJson = function () {
    ...
    .spread(function (json, deprecated, assumed) {
        ...
        if (!assumed) {
            that._jsonFile = path.join(that._config.cwd, deprecated ? deprecated : 'partm.json');
        }
...
Project.prototype._readInstalled = function () {
    ...
    return this._installed = Q.nfcall(glob, '*/.partm.json', {
        cwd: componentsDir,
        dot: true
    })
